...
class GradeBook:
    ...
    def print_current_grade(self, id):
        for student in self.students:
            if student.andrew_id == id:
                letter_grade = student.get_current_letter_grade()
                print(f"Current letter grade for {id} is: {letter_grade}")
                return
        print(f"Error: No student with id {id} found.")
        # To-Do
    ...
    def what_do_i_need(self, id, grade):
        for student in self.students:
            if student.andrew_id == id:
                req_score = student.what_do_i_need(grade)
                if req_score == -1:
                    print(f"All assignments are completed. No additional score needed.")
                elif req_score > 100:
                    print(f"Impossible to achieve {grade} as the required score is above 100.")
                else:
                    print(f"Student {id} needs an average of {req_score:.2f} on the remaining work for a(n) {grade}")
                return
        print(f"Error: No student with id {id} found.")
        # To-Do
